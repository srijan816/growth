...
// Extract feedback for primary students
...
async function importSrijanPrimaryFeedback() {
  try {
    console.log('Starting Srijan primary feedback import (non-conflicting students only)...\n');
    ...
    // Get students already imported by Srijan
    const existingQuery = `
    ...
    const existingNames = new Set(existingResult.rows.map(r => r.student_name));
    
    console.log(`Already imported by Srijan: ${existingNames.size} students\n`);
    
    // Find all Srijan's primary feedback files
    const srijanDir = '/Users/tikaram/Downloads/claude-code/student-growth/growth-compass/data/Overall/Primary/Srijan';
    ...
    const allFiles = findDocxFiles(srijanDir);
    console.log(`Found ${allFiles.length} Srijan feedback files\n`);
    
    // Process each non-conflicting student not yet imported
    let totalImported = 0;
      ...
      if (feedbackEntries.length > 0) {
        ...
        for (const entry of feedbackEntries) {
          ...
          const uniqueId = crypto.createHash('md5').update(
            `${entry.studentName}_${entry.classCode}_${entry.unit}.${entry.lesson}_${entry.index}_${path.basename(entry.filePath)}_Srijan_primary`
          ).digest('hex');
    ...
    // Summary
    console.log('\n\n' + '='.repeat(70));
    console.log('=== SRIJAN PRIMARY IMPORT SUMMARY ===');
    console.log('='.repeat(70));
    console.log(`Non-conflicting students processed: ${studentsWithFeedback}`);
    console.log(`Total entries imported: ${totalImported}`);
    
    if (processedStudents.length > 0) {
      console.log('\n\nStudents imported:');
      processedStudents.forEach(s => {
        console.log(`- ${s.name} (${s.grade}): ${s.entries} entries`);
      });
    }
    
    // Final stats for Srijan
    const finalQuery = `
      SELECT 
    ...
    console.log('\n\nTotal Srijan primary feedback in database:');
