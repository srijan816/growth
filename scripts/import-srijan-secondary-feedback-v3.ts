    ...
    for (const [firstName, feedbackList] of feedbackByStudent) {
      ...
      for (const feedback of feedbackList) {
        try {
          ...
          // Convert unit and lesson numbers to integers
          const unitNum = parseInt(feedback.unitNumber) || 0
          const lessonNum = parseInt(feedback.lessonNumber) || 0
          ...
          await executeQuery(insertQuery, [
            ...
            unitNum,                              // unit_number (as integer)
            lessonNum,                            // lesson_number (as integer)
            feedback.topic || null,               // topic
            feedback.motion || null,              // motion
            'secondary',                          // feedback_type
            feedback.content || 'No content available', // content
            feedback.rawContent || feedback.content || '', // raw_content
            feedback.htmlContent || null,         // html_content
            ...
            feedback.instructor || 'Srijan',      // instructor
            feedback.uniqueId,                    // unique_id
