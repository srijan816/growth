...
async function importRemainingSecondaryFeedback() {
  try {
    console.log('Importing feedback for remaining secondary students...\n');
    
    // Get remaining secondary students
    const studentsQuery = `
      SELECT s.id, s.name, s.grade 
      FROM students s
      WHERE s.grade LIKE 'Grade %' 
      AND CAST(SUBSTRING(s.grade FROM 7) AS INTEGER) >= 7
      AND s.name NOT IN (
        SELECT DISTINCT student_name 
        FROM parsed_student_feedback 
        WHERE instructor = 'Srijan' AND feedback_type = 'secondary'
      )
      ORDER BY s.name
    `;
    
    const studentsResult = await pool.query(studentsQuery);
    console.log(`Found ${studentsResult.rows.length} remaining students to import\n`);
    ...
    for (const student of studentsResult.rows) {
      ...
      if (feedbackEntries.length > 0) {
        ...
        console.log(`\n${student.name} (${student.grade}): No feedback found`);
      }
    }
    ...
    console.log(`Students processed: ${studentsResult.rows.length}`);
    ...
    // Final verification
    console.log('\n\nFinal verification of all secondary students:');
    const finalQuery = `
      SELECT 
...
// Run the import
importRemainingSecondaryFeedback();